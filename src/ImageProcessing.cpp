...
int ImageProcessing(osg::Image* source, osg::Matrixd intrinsics_matrix, struct InputParameters *input_parameters, float y_offset, float laser_incline, vector<Point3f>& point_cloud_points) {
    ...
    LoadIntersectionPoints(intersections, intersection_points_left);

    inRange(image_roi_right, lbound, ubound, intersections);
    LoadIntersectionPoints(intersections, intersection_points_right);
    ...
    Prova(intersection_points_left, intrinsics, input_parameters, y_offset, laser_incline, y_start_left, LEFT, point_cloud_points);
    Prova(intersection_points_right, intrinsics, input_parameters, y_offset, laser_incline, y_start_right, RIGHT, point_cloud_points);
...
void Prova(vector<cv::Point3f> intersection_points, Mat intrinsics, struct InputParameters *input_parameters, float y_offset, float laser_incline, int y_start, bool roi, vector<Point3f>& point_cloud_points) {
    ...
    for (int i = 0; i < intersection_points.size() - 1; i++) {
        ...
        if (roi) { // left roi
            ...
            relative_pixel_position = 544-(100+pixel_position);
